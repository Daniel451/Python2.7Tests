...
# ufiltered data
u_filtered = [ i[11] for i in filter(lambda x: len(x) > 4, data) ]
# vfiltered data
v_filtered = [ i[12] for i in filter(lambda x: len(x) > 4, data) ]

# ufiltered data
u_raw = [ i[5] for i in filter(lambda x: len(x) > 4, data) ]
# vfiltered data
v_raw = [ i[6] for i in filter(lambda x: len(x) > 4, data) ]
...
datarawu = noiser(datarawu, 0.99, 1.01)
datarawv = noiser(datarawv, 0.95, 1.05)
#datau = noiser(datau, 4.0, 4.0)
#datav = noiser(datav, 4.0, 4.0)

datarawu = normalizer(datarawu, 200.0)
datarawv = normalizer(datarawv, -1000.0)
...
def plot(datau, datav, datarawu, datarawv):
    ...
    # filtered
    plt.subplot()
    plt.title("calculated goal center (raw data)")
    #    plt.ylim([ymin, ymax])
    #   plt.xlim([xmin, xmax])
    ...
    plt.xticks(np.arange(-1000, max(datarawv)+1.0, 10.0))
    plt.yticks(np.arange(-1000, max(datarawu)+1.0, 10.0))
